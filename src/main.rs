use num_bigint::{BigInt, BigUint, RandBigInt, Sign::Plus, ToBigInt, ToBigUint};
...
fn main() {
    ...
    println!("Found first prime in {}ms", now.elapsed().as_millis()); // Print the time it took
    let now = Instant::now(); // Reset the timer
                              // Find another prime number 'q', different from 'p'
    let q = find_prime(num_tries, num_bits, num_iterations, Some(&p));
    println!("Found second prime in {}ms", now.elapsed().as_millis()); // Print the time it took
    let now = Instant::now(); // Reset the timer
                              // Create the public key from 'p' and 'q'
